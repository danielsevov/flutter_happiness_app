// Mocks generated by Mockito 5.3.2 from annotations
// in happiness_app/test/presentation/page/dashboard_page_test.dart.
// Do not manually edit this file.

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'dart:async' as _i8;

import 'package:flutter_local_notifications/flutter_local_notifications.dart'
    as _i12;
import 'package:happiness_app/domain/models/happiness_report_model.dart'
    as _i18;
import 'package:happiness_app/domain/models/happiness_settings_model.dart'
    as _i2;
import 'package:happiness_app/domain/repositories/happiness_report_repo.dart'
    as _i3;
import 'package:happiness_app/domain/repositories/happiness_settings_repo.dart'
    as _i10;
import 'package:happiness_app/domain/repositories/odoo_token_repo.dart' as _i13;
import 'package:happiness_app/domain/repositories/user_repo.dart' as _i4;
import 'package:happiness_app/presentation/presenters/daily_history_presenter.dart'
    as _i6;
import 'package:happiness_app/presentation/presenters/daily_intro_presenter.dart'
    as _i17;
import 'package:happiness_app/presentation/presenters/manager_dashboard_presenter.dart'
    as _i14;
import 'package:happiness_app/presentation/presenters/settings_presenter.dart'
    as _i9;
import 'package:happiness_app/presentation/presenters/weekly_retro_presenter.dart'
    as _i20;
import 'package:happiness_app/presentation/state_management/user_details_state.dart'
    as _i5;
import 'package:happiness_app/presentation/ui/widgets/daily_introspection/happiness_report.dart'
    as _i16;
import 'package:happiness_app/presentation/views/pages/daily_history_page_view.dart'
    as _i7;
import 'package:happiness_app/presentation/views/pages/daily_intro_page_view.dart'
    as _i19;
import 'package:happiness_app/presentation/views/pages/manager_dashboard_page_view.dart'
    as _i15;
import 'package:happiness_app/presentation/views/pages/settings_page_view.dart'
    as _i11;
import 'package:happiness_app/presentation/views/pages/weekly_retro_page_view.dart'
    as _i21;
import 'package:mockito/mockito.dart' as _i1;

// ignore_for_file: type=lint
// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis
// ignore_for_file: camel_case_types
// ignore_for_file: subtype_of_sealed_class

class _FakeHappinessSettingsModel_0 extends _i1.SmartFake
    implements _i2.HappinessSettingsModel {
  _FakeHappinessSettingsModel_0(
    Object parent,
    Invocation parentInvocation,
  ) : super(
          parent,
          parentInvocation,
        );
}

class _FakeHappinessReportRepository_1 extends _i1.SmartFake
    implements _i3.HappinessReportRepository {
  _FakeHappinessReportRepository_1(
    Object parent,
    Invocation parentInvocation,
  ) : super(
          parent,
          parentInvocation,
        );
}

class _FakeUserRepository_2 extends _i1.SmartFake
    implements _i4.UserRepository {
  _FakeUserRepository_2(
    Object parent,
    Invocation parentInvocation,
  ) : super(
          parent,
          parentInvocation,
        );
}

class _FakeUserDetailsState_3 extends _i1.SmartFake
    implements _i5.UserDetailsState {
  _FakeUserDetailsState_3(
    Object parent,
    Invocation parentInvocation,
  ) : super(
          parent,
          parentInvocation,
        );
}

/// A class which mocks [DailyIntrospectionHistoryPresenter].
///
/// See the documentation for Mockito's code generation for more information.
class MockDailyIntrospectionHistoryPresenter extends _i1.Mock
    implements _i6.DailyIntrospectionHistoryPresenter {
  MockDailyIntrospectionHistoryPresenter() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i2.HappinessSettingsModel get settings => (super.noSuchMethod(
        Invocation.getter(#settings),
        returnValue: _FakeHappinessSettingsModel_0(
          this,
          Invocation.getter(#settings),
        ),
      ) as _i2.HappinessSettingsModel);
  @override
  set settings(_i2.HappinessSettingsModel? _settings) => super.noSuchMethod(
        Invocation.setter(
          #settings,
          _settings,
        ),
        returnValueForMissingStub: null,
      );
  @override
  bool get repositoriesAttached => (super.noSuchMethod(
        Invocation.getter(#repositoriesAttached),
        returnValue: false,
      ) as bool);
  @override
  set repositoriesAttached(bool? _repositoriesAttached) => super.noSuchMethod(
        Invocation.setter(
          #repositoriesAttached,
          _repositoriesAttached,
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attachRepositories(
          _i3.HappinessReportRepository? happinessReportRepository) =>
      super.noSuchMethod(
        Invocation.method(
          #attachRepositories,
          [happinessReportRepository],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attach(_i7.DailyIntrospectionHistoryPageView? view) =>
      super.noSuchMethod(
        Invocation.method(
          #attach,
          [view],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void detach() => super.noSuchMethod(
        Invocation.method(
          #detach,
          [],
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i8.Future<void> fetchReports({
    int? pageLimit,
    int? currentPageIndex,
    required bool? fetchDaily,
  }) =>
      (super.noSuchMethod(
        Invocation.method(
          #fetchReports,
          [],
          {
            #pageLimit: pageLimit,
            #currentPageIndex: currentPageIndex,
            #fetchDaily: fetchDaily,
          },
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
}

/// A class which mocks [SettingsPresenter].
///
/// See the documentation for Mockito's code generation for more information.
class MockSettingsPresenter extends _i1.Mock implements _i9.SettingsPresenter {
  MockSettingsPresenter() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i2.HappinessSettingsModel get settings => (super.noSuchMethod(
        Invocation.getter(#settings),
        returnValue: _FakeHappinessSettingsModel_0(
          this,
          Invocation.getter(#settings),
        ),
      ) as _i2.HappinessSettingsModel);
  @override
  set settings(_i2.HappinessSettingsModel? _settings) => super.noSuchMethod(
        Invocation.setter(
          #settings,
          _settings,
        ),
        returnValueForMissingStub: null,
      );
  @override
  bool get isFirst => (super.noSuchMethod(
        Invocation.getter(#isFirst),
        returnValue: false,
      ) as bool);
  @override
  set isFirst(bool? _isFirst) => super.noSuchMethod(
        Invocation.setter(
          #isFirst,
          _isFirst,
        ),
        returnValueForMissingStub: null,
      );
  @override
  bool get repositoriesAttached => (super.noSuchMethod(
        Invocation.getter(#repositoriesAttached),
        returnValue: false,
      ) as bool);
  @override
  set repositoriesAttached(bool? _repositoriesAttached) => super.noSuchMethod(
        Invocation.setter(
          #repositoriesAttached,
          _repositoriesAttached,
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attachRepositories(
          _i10.HappinessSettingsRepository? happinessSettingsRepository) =>
      super.noSuchMethod(
        Invocation.method(
          #attachRepositories,
          [happinessSettingsRepository],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attach(_i11.SettingsPageView? view) => super.noSuchMethod(
        Invocation.method(
          #attach,
          [view],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void detach() => super.noSuchMethod(
        Invocation.method(
          #detach,
          [],
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i8.Future<void> fetchSettings() => (super.noSuchMethod(
        Invocation.method(
          #fetchSettings,
          [],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<void> changeLocaleSettings(String? locale) => (super.noSuchMethod(
        Invocation.method(
          #changeLocaleSettings,
          [locale],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<void> changeNotificationsSettings({
    required bool? monday,
    required bool? tuesday,
    required bool? wednesday,
    required bool? thursday,
    required bool? friday,
    required bool? saturday,
    required bool? sunday,
    String? timeOfTheDay,
    required _i12.Day? weeklyDayOfTheWeek,
    String? weeklyTimeOfTheDay,
  }) =>
      (super.noSuchMethod(
        Invocation.method(
          #changeNotificationsSettings,
          [],
          {
            #monday: monday,
            #tuesday: tuesday,
            #wednesday: wednesday,
            #thursday: thursday,
            #friday: friday,
            #saturday: saturday,
            #sunday: sunday,
            #timeOfTheDay: timeOfTheDay,
            #weeklyDayOfTheWeek: weeklyDayOfTheWeek,
            #weeklyTimeOfTheDay: weeklyTimeOfTheDay,
          },
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<void> changeDataSettings({required bool? canShare}) =>
      (super.noSuchMethod(
        Invocation.method(
          #changeDataSettings,
          [],
          {#canShare: canShare},
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
}

/// A class which mocks [HappinessSettingsRepository].
///
/// See the documentation for Mockito's code generation for more information.
class MockHappinessSettingsRepository extends _i1.Mock
    implements _i10.HappinessSettingsRepository {
  MockHappinessSettingsRepository() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i8.Future<_i2.HappinessSettingsModel> getForEmployee(int? employeeId) =>
      (super.noSuchMethod(
        Invocation.method(
          #getForEmployee,
          [employeeId],
        ),
        returnValue: _i8.Future<_i2.HappinessSettingsModel>.value(
            _FakeHappinessSettingsModel_0(
          this,
          Invocation.method(
            #getForEmployee,
            [employeeId],
          ),
        )),
      ) as _i8.Future<_i2.HappinessSettingsModel>);
  @override
  _i8.Future<_i2.HappinessSettingsModel> getMySettings() => (super.noSuchMethod(
        Invocation.method(
          #getMySettings,
          [],
        ),
        returnValue: _i8.Future<_i2.HappinessSettingsModel>.value(
            _FakeHappinessSettingsModel_0(
          this,
          Invocation.method(
            #getMySettings,
            [],
          ),
        )),
      ) as _i8.Future<_i2.HappinessSettingsModel>);
  @override
  _i8.Future<_i2.HappinessSettingsModel> update(
          _i2.HappinessSettingsModel? dailyIntrospection) =>
      (super.noSuchMethod(
        Invocation.method(
          #update,
          [dailyIntrospection],
        ),
        returnValue: _i8.Future<_i2.HappinessSettingsModel>.value(
            _FakeHappinessSettingsModel_0(
          this,
          Invocation.method(
            #update,
            [dailyIntrospection],
          ),
        )),
      ) as _i8.Future<_i2.HappinessSettingsModel>);
}

/// A class which mocks [OdooTokenRepository].
///
/// See the documentation for Mockito's code generation for more information.
class MockOdooTokenRepository extends _i1.Mock
    implements _i13.OdooTokenRepository {
  MockOdooTokenRepository() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i8.Future<void> clearOdooToken() => (super.noSuchMethod(
        Invocation.method(
          #clearOdooToken,
          [],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<void> saveToken(String? value) => (super.noSuchMethod(
        Invocation.method(
          #saveToken,
          [value],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<String?> getOdooToken() => (super.noSuchMethod(
        Invocation.method(
          #getOdooToken,
          [],
        ),
        returnValue: _i8.Future<String?>.value(),
      ) as _i8.Future<String?>);
}

/// A class which mocks [ManagerDashboardPresenter].
///
/// See the documentation for Mockito's code generation for more information.
class MockManagerDashboardPresenter extends _i1.Mock
    implements _i14.ManagerDashboardPresenter {
  MockManagerDashboardPresenter() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i3.HappinessReportRepository get happinessReportRepository =>
      (super.noSuchMethod(
        Invocation.getter(#happinessReportRepository),
        returnValue: _FakeHappinessReportRepository_1(
          this,
          Invocation.getter(#happinessReportRepository),
        ),
      ) as _i3.HappinessReportRepository);
  @override
  set happinessReportRepository(
          _i3.HappinessReportRepository? _happinessReportRepository) =>
      super.noSuchMethod(
        Invocation.setter(
          #happinessReportRepository,
          _happinessReportRepository,
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i4.UserRepository get userRepository => (super.noSuchMethod(
        Invocation.getter(#userRepository),
        returnValue: _FakeUserRepository_2(
          this,
          Invocation.getter(#userRepository),
        ),
      ) as _i4.UserRepository);
  @override
  set userRepository(_i4.UserRepository? _userRepository) => super.noSuchMethod(
        Invocation.setter(
          #userRepository,
          _userRepository,
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i2.HappinessSettingsModel get settings => (super.noSuchMethod(
        Invocation.getter(#settings),
        returnValue: _FakeHappinessSettingsModel_0(
          this,
          Invocation.getter(#settings),
        ),
      ) as _i2.HappinessSettingsModel);
  @override
  set settings(_i2.HappinessSettingsModel? _settings) => super.noSuchMethod(
        Invocation.setter(
          #settings,
          _settings,
        ),
        returnValueForMissingStub: null,
      );
  @override
  bool get repositoriesAttached => (super.noSuchMethod(
        Invocation.getter(#repositoriesAttached),
        returnValue: false,
      ) as bool);
  @override
  set repositoriesAttached(bool? _repositoriesAttached) => super.noSuchMethod(
        Invocation.setter(
          #repositoriesAttached,
          _repositoriesAttached,
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attachRepositories(
    _i3.HappinessReportRepository? reportRepository,
    _i4.UserRepository? userRepo,
  ) =>
      super.noSuchMethod(
        Invocation.method(
          #attachRepositories,
          [
            reportRepository,
            userRepo,
          ],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attach(_i15.ManagerDashboardPageView? view) => super.noSuchMethod(
        Invocation.method(
          #attach,
          [view],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void detach() => super.noSuchMethod(
        Invocation.method(
          #detach,
          [],
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i8.Future<void> fetchData({required bool? fetchDaily}) =>
      (super.noSuchMethod(
        Invocation.method(
          #fetchData,
          [],
          {#fetchDaily: fetchDaily},
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  List<_i16.HappinessReport> mergeReportsByDate(
          List<_i16.HappinessReport>? reports) =>
      (super.noSuchMethod(
        Invocation.method(
          #mergeReportsByDate,
          [reports],
        ),
        returnValue: <_i16.HappinessReport>[],
      ) as List<_i16.HappinessReport>);
}

/// A class which mocks [DailyIntrospectionPresenter].
///
/// See the documentation for Mockito's code generation for more information.
class MockDailyIntrospectionPresenter extends _i1.Mock
    implements _i17.DailyIntrospectionPresenter {
  MockDailyIntrospectionPresenter() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i5.UserDetailsState get userDetails => (super.noSuchMethod(
        Invocation.getter(#userDetails),
        returnValue: _FakeUserDetailsState_3(
          this,
          Invocation.getter(#userDetails),
        ),
      ) as _i5.UserDetailsState);
  @override
  set todaysReport(_i18.HappinessReportModel? _todaysReport) =>
      super.noSuchMethod(
        Invocation.setter(
          #todaysReport,
          _todaysReport,
        ),
        returnValueForMissingStub: null,
      );
  @override
  bool get repositoriesAttached => (super.noSuchMethod(
        Invocation.getter(#repositoriesAttached),
        returnValue: false,
      ) as bool);
  @override
  set repositoriesAttached(bool? _repositoriesAttached) => super.noSuchMethod(
        Invocation.setter(
          #repositoriesAttached,
          _repositoriesAttached,
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attachRepositories(
          _i3.HappinessReportRepository? happinessReportRepository) =>
      super.noSuchMethod(
        Invocation.method(
          #attachRepositories,
          [happinessReportRepository],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attach(_i19.DailyIntrospectionPageView? view) => super.noSuchMethod(
        Invocation.method(
          #attach,
          [view],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void detach() => super.noSuchMethod(
        Invocation.method(
          #detach,
          [],
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i8.Future<void> fetchReport(String? date) => (super.noSuchMethod(
        Invocation.method(
          #fetchReport,
          [date],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<void> saveChanges(
    double? happinessLevel,
    double? sadnessLevel,
    double? angerLevel,
    double? fearLevel,
    String? accomplishments,
    String? contributions,
    String? insight,
    String? date,
  ) =>
      (super.noSuchMethod(
        Invocation.method(
          #saveChanges,
          [
            happinessLevel,
            sadnessLevel,
            angerLevel,
            fearLevel,
            accomplishments,
            contributions,
            insight,
            date,
          ],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
}

/// A class which mocks [WeeklyRetrospectionPresenter].
///
/// See the documentation for Mockito's code generation for more information.
class MockWeeklyRetrospectionPresenter extends _i1.Mock
    implements _i20.WeeklyRetrospectionPresenter {
  MockWeeklyRetrospectionPresenter() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i5.UserDetailsState get userDetails => (super.noSuchMethod(
        Invocation.getter(#userDetails),
        returnValue: _FakeUserDetailsState_3(
          this,
          Invocation.getter(#userDetails),
        ),
      ) as _i5.UserDetailsState);
  @override
  set currentWeekReport(_i18.HappinessReportModel? _currentWeekReport) =>
      super.noSuchMethod(
        Invocation.setter(
          #currentWeekReport,
          _currentWeekReport,
        ),
        returnValueForMissingStub: null,
      );
  @override
  bool get repositoriesAttached => (super.noSuchMethod(
        Invocation.getter(#repositoriesAttached),
        returnValue: false,
      ) as bool);
  @override
  set repositoriesAttached(bool? _repositoriesAttached) => super.noSuchMethod(
        Invocation.setter(
          #repositoriesAttached,
          _repositoriesAttached,
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attachRepositories(
          _i3.HappinessReportRepository? happinessReportRepository) =>
      super.noSuchMethod(
        Invocation.method(
          #attachRepositories,
          [happinessReportRepository],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void attach(_i21.WeeklyRetrospectionPageView? view) => super.noSuchMethod(
        Invocation.method(
          #attach,
          [view],
        ),
        returnValueForMissingStub: null,
      );
  @override
  void detach() => super.noSuchMethod(
        Invocation.method(
          #detach,
          [],
        ),
        returnValueForMissingStub: null,
      );
  @override
  _i8.Future<void> fetchReport(
    int? weekNumber,
    int? year,
  ) =>
      (super.noSuchMethod(
        Invocation.method(
          #fetchReport,
          [
            weekNumber,
            year,
          ],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
  @override
  _i8.Future<void> saveChanges(
    int? weeklyRating,
    String? feedback,
    String? insight,
    int? weekNumber,
    int? year,
  ) =>
      (super.noSuchMethod(
        Invocation.method(
          #saveChanges,
          [
            weeklyRating,
            feedback,
            insight,
            weekNumber,
            year,
          ],
        ),
        returnValue: _i8.Future<void>.value(),
        returnValueForMissingStub: _i8.Future<void>.value(),
      ) as _i8.Future<void>);
}
